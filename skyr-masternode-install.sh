...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='skyrcoin.conf'
CONFIGFOLDER='/root/.skyrcoin'
COIN_DAEMON='skyrcoind'
COIN_CLI='skyrcoin-cli'
COIN_TX='skyrcoin-tx'
COIN_PATH='/usr/local/bin/'
OS_VERSION=$(lsb_release -d)
COIN_TGZP='https://github.com/SkynetResearchProject/Skyrcoin/releases/download/v2.0.0.9-cs/skyrcoin-2.0.0.9-cs-linux-gnu-x64.tar.gz'
COIN_BOOTSTRAP='https://github.com/SkynetResearchProject/Skyrcoin/releases/download/v2.0.0.8b/bootstrap.tar.gz'
COIN_BOOTSTRAP_NAME=$(echo $COIN_BOOTSTRAP | awk -F'/' '{print $NF}')
COIN_TGZ=$(echo $COIN_TGZP | awk -F'/' '{print $NF}')
COIN_NAME='skyrcoin'
COIN_NAME_OLD='skyrcoind'
PROJECT_NAME='skyrcoin (SKYR)'
COIN_EXPLORER='http://185.177.218.246:3001'
COIN_PORT=16888
RPC_PORT=16890
...
purgeOldInstallation() {
	...
	pkill -f -9 $COIN_DAEMON > /dev/null 2>&1
...
function download_node() {
  echo -e "${GREEN}Installing VPS ${BLUE}$PROJECT_NAME ${GREEN}Daemon${NC}"
  echo -e "${GREEN}Downloading and Installing VPS ${BLUE}$PROJECT_NAME ${GREEN}Daemon${NC}"
  cd $TMP_FOLDER >/dev/null 2>&1
  wget -q $COIN_TGZP
  #compile_error
  tar -xvf $COIN_TGZ >/dev/null 2>&1
  rm -r $COIN_TGZ >/dev/null 2>&1

  ##compile_error
  chmod +x $COIN_DAEMON >/dev/null 2>&1       #or   chmod +x ./$COIN_DAEMON >/dev/null 2>&1  # not tested
...
cat << EOF > /root/skyrcoin-general-info
echo -e "\n\n${BLUE}=======================================================================================================${NC}\n"
echo -e "${GREEN}$PROJECT_NAME General Info: ${NC}"
$COIN_CLI getinfo
echo -e "\n${BLUE}=======================================================================================================${NC}\n"
EOF
chmod +x /root/skyrcoin-general-info

cat << EOF > /root/skyrcoin-fee-info
echo -e "\n\n${BLUE}=======================================================================================================${NC}\n"
echo -e "${GREEN}$PROJECT_NAME Fee Info: ${NC}"
$COIN_CLI getfeeinfo 100
echo -e "\n${BLUE}=======================================================================================================${NC}\n"
EOF
chmod +x /root/skyrcoin-fee-info

cat << EOF > /root/skyrcoin-networkinfo
echo -e "\n\n${BLUE}=======================================================================================================${NC}\n"
echo -e "${GREEN}$PROJECT_NAME Network Info: ${NC}"
$COIN_CLI getnetworkinfo
echo -e "\n${BLUE}=======================================================================================================${NC}\n"
EOF
chmod +x /root/skyrcoin-networkinfo

cat << EOF > /root/skyrcoin-masternode-stats
echo -e "\n\n${BLUE}=======================================================================================================${NC}\n"
...
echo -e "\n${BLUE}=======================================================================================================${NC}\n"
EOF
chmod +x /root/skyrcoin-masternode-stats
...
function important_information() {
 ...
 echo -e "You can also use ${RED}skyrcoin-cli getinfo${NC}, ${RED}skyrcoin-cli getblockcount${NC}, ${RED}skyrcoin-cli getmasternodewinners${NC} and ${RED}skyrcoin-cli getblockchaininfo${NC} commands for a nice looking infos.${NC}"
 echo -e "${BLUE}================================================================================================================================${NC}"
...
apt-get -y install nano >/dev/null 2>&1
